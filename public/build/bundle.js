...
var app = (function () {
    ...
    function create_fragment(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(script, file, 107, 1, 3512);
    			attr_dev(canvas, "id", "renderCanvas");
    			add_location(canvas, file, 112, 1, 3716);
    			add_location(main, file, 111, 0, 3708);
    		},
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	Object.keys($$props).forEach(key => {
    		if (!~writable_props.indexOf(key) && key.slice(0, 2) !== "$$") console_1.warn(`<vr-video> was created with unknown prop '${key}'`);
    	});
    ...
    class Vr_video extends SvelteElement {
    	constructor(options) {
    		super();
    		this.shadowRoot.innerHTML = `<style>#renderCanvas{width:100%;height:100%;touch-action:none}</style>`;
